...
# functions
function audio_checkout {
	# clean before
	rm -rf $WORKSPACE/$SOURCES_REPONAME
	# clone
	cd $WORKSPACE
	time git clone git@wpp.isd.dp.ua:irls/$SOURCES_REPONAME.git
	...
	# operations for git-annex
	mkdir -p $SOURCES_CURRENT_CODE_PATH/$SOURCES_HASHCOMMIT/.git
	mv $WORKSPACE/$SOURCES_REPONAME/.git/annex $SOURCES_CURRENT_CODE_PATH/$SOURCES_HASHCOMMIT/.git/
	chmod -Rf 777 $SOURCES_CURRENT_CODE_PATH/$SOURCES_HASHCOMMIT/.git/annex
	chmod -Rf 777 $SOURCES_CURRENT_CODE_PATH/$SOURCES_HASHCOMMIT/.git/annex
}
...
if [ "$(ls -A $SOURCES_CURRENT_CODE_PATH/$SOURCES_HASHCOMMIT)" ]; then
        ...
        if find $SOURCES_CURRENT_CODE_PATH -type c -name *.mp3; then
	...
	if [ $PROCESSOR_BRANCHNAME = "audio" ]; then
        ...
        # clean before
        rm -rf $WORKSPACE/$SOURCES_REPONAME
        # clone, checkout default master
        git clone git@wpp.isd.dp.ua:irls/$SOURCES_REPONAME\.git
        cd $WORKSPACE/$SOURCES_REPONAME
        # shallow clone
