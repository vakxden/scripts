...
function search_and_copy {
        ...
        # if path to artifacts directory contain word "stage" -> search apk-files in artifacts directory for CURRENT-environment
        if [ -n "$(echo "$1" | grep stage)" ]; then
                echo contain stage;
                find_stag=$(find $1 -name *$i*.apk) > /dev/null 2>&1
                if [ ! -z "$find_stag" ]; then
                        echo "apk-file in $PWD exist" && echo "it is $find_stag"
                else
                        echo "apk-file in $PWD not exists"
                        find=$(find $2 -name *$i*.apk) > /dev/null 2>&1
                        if [ ! -z "$find" ]; then
                                echo PWD=$PWD
...
if [ "$mark" = "all" ] || [ "$mark" = "initiate-android" ]; then
        ...
        if [ "$dest" = "DEVELOPMENT" ]; then
                ...
                do
                        ...
                        # search apk-files, if not exists - copy to artifacts dir
                ...
                do
                        ...
                        # search apk-files, if not exists - copy from artifacts dir to stage artifacts dir
