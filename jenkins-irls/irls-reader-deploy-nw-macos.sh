...
DIR_ZIP=/var/lib/jenkins/jobs/irls-reader-initiate-nw-macos/builds/lastSuccessfulBuild/archive/
...
function search_and_copy {
	...
	if [ -n "$(echo "$1" | grep stage)" ]; then
		echo contain stage;
		find_stag=$(find $1 -name $BRANCH*FFA_Reader*$i-macos*.zip) > /dev/null 2>&1
		if [ ! -z "$find_stag" ]; then
			echo "nw-macos zip file in $PWD exist" && echo "it is $find_stag"
		else
			echo "nw-macos zip file in $PWD not exists"
			find=$(find $2 -name $BRANCH*FFA_Reader*$i-macos*.zip) > /dev/null 2>&1
			if [ ! -z "$find" ]; then
				echo PWD=$PWD
				cp $find . && echo "copying file $find to PWD=$PWD"
			fi
		fi
		# else -> search zip-files in directory when jenkins save jobs artifacts
	else
		zip_file=$(find $DIR_ZIP -name $BRANCH*FFA_Reader*$i-macos*.zip)
		if [ ! -f "$zip_file" ]; then
			echo "nw-macos zip file $zip_file in $DIR_ZIP not exists"
		else
			echo "find nw-macos zip-file $zip_file"
			cp $zip_file $1
		fi
	fi
}
...
###
### If the variable $mark is equal to the value of "all" or "initiate-nw-macos", then perform the body of this script 
###
if [ "$mark" = "all" ] || [ "$mark" = "initiate-nw-macos" ]; then
	###
	### Body
	###
	if [ "$dest" = "DEVELOPMENT" ]; then
		...
		do
			...
			# search node-webkit for MacOS (nw-macos) zip-files, if not exists - copy to artifacts dir
		...
		do
			...
			# search node-webkit for MacOS (nw-macos) zip-files, if not exists - copy from artifacts dir to stage artifacts dir
		...
		do
			...
			ssh dvac@devzone.dp.ua "
				...
				then
					...
					rm -rf  ~/irls-reader-artifacts/${combineArray[$i]}/packages/artifact/$BRANCH*FFA_Reader*$i-macos*.zip
				fi"
			ARTIFACTS_DIR=$STAGE_ART_PATH/${combineArray[$i]}/packages/artifacts
			if [ -f $ARTIFACTS_DIR/$BRANCH*FFA_Reader*$i-macos*.zip ]; then
				scp $ARTIFACTS_DIR/$BRANCH*FFA_Reader*$i-macos*.zip dvac@devzone.dp.ua:~/irls-reader-artifacts/${combineArray[$i]}/packages/art/
			fi
			ssh dvac@devzone.dp.ua "
